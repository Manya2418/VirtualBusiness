...
const OrderCard = ({ order, onRemove }) => {
  ...
  const handleRemoveOrder = async () => {
    setLoading(true);
    try {
      await axios.delete(`http://localhost:4000/order/delete?id=${order._id}`);
      onRemove(order._id);
  ...
  return (
    <>
      <div className="bg-white shadow-md rounded-lg p-5 mb-6 w-full max-w-full">
      <div className="flex justify-between items-center mb-2 w-full">
        ...
        <h2 className="text-md text-teal-700 mb-1">Shop Name: {order.restaurantName}</h2>
