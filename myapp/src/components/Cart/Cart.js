...
export const Cart = () => {
  ...
  const handlePayment = async () => {
    ...
    setLoading(true);
    try {
      await axios.post('http://localhost:4000/order/orderdetails', orderData);
      const res = await axios.post('http://localhost:4000/order/create-checkout-session', orderData);
